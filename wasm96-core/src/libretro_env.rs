...
use libretro_sys::{
    EnvironmentFn, GameGeometry, ENVIRONMENT_SET_GEOMETRY, ENVIRONMENT_SET_PIXEL_FORMAT,
};
...
pub fn negotiate_pixel_format(env: Option<EnvironmentFn>) -> bool {
    ...
    unsafe { env(ENVIRONMENT_SET_PIXEL_FORMAT, (&raw mut fmt) as *mut _ as *mut c_void) }
}
...
pub fn maybe_emit_set_geometry(env: Option<EnvironmentFn>) -> bool {
    ...
    let ok = unsafe { env(ENVIRONMENT_SET_GEOMETRY, (&raw mut geom) as *mut _ as *mut c_void) };
